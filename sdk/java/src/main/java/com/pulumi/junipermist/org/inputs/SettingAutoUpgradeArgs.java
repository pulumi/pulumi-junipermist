// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.junipermist.org.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Boolean;
import java.lang.String;
import java.util.Map;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class SettingAutoUpgradeArgs extends com.pulumi.resources.ResourceArgs {

    public static final SettingAutoUpgradeArgs Empty = new SettingAutoUpgradeArgs();

    /**
     * Per-AP-model firmware versions or channels used for auto-upgrade
     * 
     */
    @Import(name="customVersions")
    private @Nullable Output<Map<String,String>> customVersions;

    /**
     * @return Per-AP-model firmware versions or channels used for auto-upgrade
     * 
     */
    public Optional<Output<Map<String,String>>> customVersions() {
        return Optional.ofNullable(this.customVersions);
    }

    /**
     * Day of the week for the AP auto-upgrade maintenance window
     * 
     */
    @Import(name="dayOfWeek")
    private @Nullable Output<String> dayOfWeek;

    /**
     * @return Day of the week for the AP auto-upgrade maintenance window
     * 
     */
    public Optional<Output<String>> dayOfWeek() {
        return Optional.ofNullable(this.dayOfWeek);
    }

    /**
     * Whether AP auto-upgrade is enabled. Note that Mist may auto-upgrade APs if the running version is no longer supported.
     * 
     */
    @Import(name="enabled")
    private @Nullable Output<Boolean> enabled;

    /**
     * @return Whether AP auto-upgrade is enabled. Note that Mist may auto-upgrade APs if the running version is no longer supported.
     * 
     */
    public Optional<Output<Boolean>> enabled() {
        return Optional.ofNullable(this.enabled);
    }

    /**
     * `any` or HH:MM (24-hour format). Upgrade will happen within up to 1 hour from this time.
     * 
     */
    @Import(name="timeOfDay")
    private @Nullable Output<String> timeOfDay;

    /**
     * @return `any` or HH:MM (24-hour format). Upgrade will happen within up to 1 hour from this time.
     * 
     */
    public Optional<Output<String>> timeOfDay() {
        return Optional.ofNullable(this.timeOfDay);
    }

    /**
     * Firmware release channel or specific version used for AP auto-upgrade
     * 
     */
    @Import(name="version")
    private @Nullable Output<String> version;

    /**
     * @return Firmware release channel or specific version used for AP auto-upgrade
     * 
     */
    public Optional<Output<String>> version() {
        return Optional.ofNullable(this.version);
    }

    private SettingAutoUpgradeArgs() {}

    private SettingAutoUpgradeArgs(SettingAutoUpgradeArgs $) {
        this.customVersions = $.customVersions;
        this.dayOfWeek = $.dayOfWeek;
        this.enabled = $.enabled;
        this.timeOfDay = $.timeOfDay;
        this.version = $.version;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(SettingAutoUpgradeArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private SettingAutoUpgradeArgs $;

        public Builder() {
            $ = new SettingAutoUpgradeArgs();
        }

        public Builder(SettingAutoUpgradeArgs defaults) {
            $ = new SettingAutoUpgradeArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param customVersions Per-AP-model firmware versions or channels used for auto-upgrade
         * 
         * @return builder
         * 
         */
        public Builder customVersions(@Nullable Output<Map<String,String>> customVersions) {
            $.customVersions = customVersions;
            return this;
        }

        /**
         * @param customVersions Per-AP-model firmware versions or channels used for auto-upgrade
         * 
         * @return builder
         * 
         */
        public Builder customVersions(Map<String,String> customVersions) {
            return customVersions(Output.of(customVersions));
        }

        /**
         * @param dayOfWeek Day of the week for the AP auto-upgrade maintenance window
         * 
         * @return builder
         * 
         */
        public Builder dayOfWeek(@Nullable Output<String> dayOfWeek) {
            $.dayOfWeek = dayOfWeek;
            return this;
        }

        /**
         * @param dayOfWeek Day of the week for the AP auto-upgrade maintenance window
         * 
         * @return builder
         * 
         */
        public Builder dayOfWeek(String dayOfWeek) {
            return dayOfWeek(Output.of(dayOfWeek));
        }

        /**
         * @param enabled Whether AP auto-upgrade is enabled. Note that Mist may auto-upgrade APs if the running version is no longer supported.
         * 
         * @return builder
         * 
         */
        public Builder enabled(@Nullable Output<Boolean> enabled) {
            $.enabled = enabled;
            return this;
        }

        /**
         * @param enabled Whether AP auto-upgrade is enabled. Note that Mist may auto-upgrade APs if the running version is no longer supported.
         * 
         * @return builder
         * 
         */
        public Builder enabled(Boolean enabled) {
            return enabled(Output.of(enabled));
        }

        /**
         * @param timeOfDay `any` or HH:MM (24-hour format). Upgrade will happen within up to 1 hour from this time.
         * 
         * @return builder
         * 
         */
        public Builder timeOfDay(@Nullable Output<String> timeOfDay) {
            $.timeOfDay = timeOfDay;
            return this;
        }

        /**
         * @param timeOfDay `any` or HH:MM (24-hour format). Upgrade will happen within up to 1 hour from this time.
         * 
         * @return builder
         * 
         */
        public Builder timeOfDay(String timeOfDay) {
            return timeOfDay(Output.of(timeOfDay));
        }

        /**
         * @param version Firmware release channel or specific version used for AP auto-upgrade
         * 
         * @return builder
         * 
         */
        public Builder version(@Nullable Output<String> version) {
            $.version = version;
            return this;
        }

        /**
         * @param version Firmware release channel or specific version used for AP auto-upgrade
         * 
         * @return builder
         * 
         */
        public Builder version(String version) {
            return version(Output.of(version));
        }

        public SettingAutoUpgradeArgs build() {
            return $;
        }
    }

}
