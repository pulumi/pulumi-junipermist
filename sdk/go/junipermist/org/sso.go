...
//
// The SSO Metadata can be retrieved with the `org.getSsoMetadata` data source.
//
// ## Example Usage
//
// ```go
...
type Sso struct {
	...
	ScimSecretToken pulumi.StringPtrOutput `pulumi:"scimSecretToken"`
}
...
// if `idpType`==`oauth`, indicates if SCIM provisioning is enabled for the OAuth IDP
func (o SsoOutput) ScimEnabled() pulumi.BoolOutput {
	return o.ApplyT(func(v *Sso) pulumi.BoolOutput { return v.ScimEnabled }).(pulumi.BoolOutput)
}

// if `idpType`==`oauth`, scimSecretToken (generated by caller, crypto-random) is used as the Bearer token in the
// Authorization header of SCIM provisioning requests by the IDP
func (o SsoOutput) ScimSecretToken() pulumi.StringPtrOutput {
	return o.ApplyT(func(v *Sso) pulumi.StringPtrOutput { return v.ScimSecretToken }).(pulumi.StringPtrOutput)
}
